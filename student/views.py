...
def upload_file(request):
    ...
    # 规定上传目录
    upload_url = os.path.join('/upload_files/', 'attachment', curttime)
    print(upload_url)
    # 判断文件夹是否存在
    ...
    if file:
        ...
        teacher = models.Teacher.objects.get(id=request.session['user_id'])
        models.FileInfo.objects.create(file_name=file.name, file_size=1,
                             file_path=os.path.join(upload_url, file.name),tea_id =teacher )
        ...
        # 返回文件的URl
        file_upload_url = '/upload_files/' + 'attachment/' + curttime + '/' + finally_name
        # 构建返回值
        response_data = {}
...
def fileList(request):
    if request.method == 'GET':
        ...
        files = models.FileInfo.objects.filter(tea_id=teacherid)
        rows = []
        total = len(files)
        for dnfile in files:
            rows.append({'id':dnfile.id,'file_name':dnfile.file_name})
        data = {"total": total, "rows": rows}
        return JsonResponse(data)
def delete_file(request):
#通过id查找到文件，删除存储文件及数据库中记录
    fid = request.POST.get('id')
    fileid = models.FileInfo.objects.get(id=fid)
    os.remove(fileid.file_path)
    fileid.delete()
    data = {"delete": fid}
    return JsonResponse(data)
...
def download_file(request):
    fid = request.POST.get('id')
    fileid = models.FileInfo.objects.get(id=fid)
    file = open(fileid.file_path, 'rb')
    print(fileid)
    response = FileResponse(file)
    response['Content-Type'] = 'application/octet-stream'
    response['Content-Disposition'] = 'attachment;filename="{}"'.format(fileid.file_name)
    data = {"download": fid}
   # return JsonResponse(data)
    return response
